...
export function WhatWeDoSection() {
  return (
    <section
      id="what-we-do"
      className="relative bg-surface-dark/80 py-20 overflow-hidden"
    >
      {/* Background blobs */}
      <div
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 animate-on-scroll">
        ...
        <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3" role="list">
          {capabilities.map((item, index) => (
            <li
              key={index}
              className={`flex items-start gap-4 rounded-2xl border border-white/10 bg-white/5 p-6 hover:bg-white/10 hover:border-accent/40 transition-all duration-200${capabilities.length === 5 && index === 4 ? ' lg:col-start-2' : ''}`}
            >
              <span
