          ...
          {projects.map((project) => {
            ...
            return (
              <CardWrapper
                ...
                className={`group bg-surface rounded-2xl border border-border overflow-hidden card-hover hover:border-border-hover flex flex-col${project.url ? ' cursor-pointer' : ''}`}
                {...linkProps}
              >
                <div className="relative w-full aspect-[4/3] overflow-hidden">
                  ...
                  >
                    {project.url && (
                      <span className="text-white text-sm font-medium border border-white/30 rounded-full px-4 py-2">
                        View project →
                      </span>
                    )}
                  </div>
